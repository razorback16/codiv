...
use slate_common::messages::{DaemonMessage, RiskLevel};
...
async fn request_confirmation(
    ...
    let frame = slate_common::messages::frame_message(&msg).map_err(|e| e.to_string())?;
...
fn send_permission_outcome(ctx: &PermissionContext, tool_name: &str, granted: bool, reason: &str) {
    use slate_common::messages::DaemonMessage;
