...
name: slate-aisdk
description: >
  Guide for building with the aisdk library (razorback16/aisdk) in the Slate agent system.
  ...
  - Working on slated daemon code (crates/slated/) especially agent/, config, or streaming logic
  ...
  agent loop, or LLM integration in the context of Slate, use this skill.
---

# Slate aisdk Integration Guide

This skill covers the **aisdk** library (a Rust Vercel AI SDK-inspired multi-provider LLM abstraction)
and how it integrates with the **slated** daemon in the Slate terminal agent project.

## Project Layout

```
slate-agent/                          # Main Slate project
├── crates/
│   ├── slate/                        # Rust TUI client (ratatui + crossterm)
│   ├── slated/                       # Async daemon — where aisdk is consumed
│   │   └── src/
...
│   ├── slate-common/                 # Shared types: ClientMessage, DaemonMessage, StreamChunk
│   └── slate-tools/                  # Tool implementations
│       └── src/tools/                # bash.rs, read.rs, write.rs, edit.rs, glob.rs, grep.rs

slate-agent-skills/aisdk/             # The aisdk library itself
├── src/
...
For multi-turn conversations (what slated does):
```rust
...
The system message is set separately via `.system()` on the request builder, not in the messages vector. In slated's `Agent::build_messages()`, the system prompt is kept separate and passed via the request builder.
...
**Manual Tool construction** (what slated does for tools needing captured state):
```rust
...
### Adding a New Tool to Slated (the full slate-specific flow)

Slated's tools span three layers. Follow all three when adding a new tool:

**Step 1: Create the tool module in `crates/slate-tools/src/tools/`**

Each tool is a module exporting an input struct and an `execute` function:

```rust
// crates/slate-tools/src/tools/search.rs
use serde::Deserialize;
...
**Step 2: Register the module in `crates/slate-tools/src/tools/mod.rs`**

```rust
pub mod search;  // add alongside bash, edit, glob, grep, read, write
```

**Step 3: Add to `build_tools()` in `crates/slated/src/agent/tools.rs`**

Slated uses a `make_tool` helper that combines the name, description, schemars-derived
schema, and execute function into an aisdk `Tool`:

```rust
use slate_tools::tools::search;  // add to imports
...
### 6. How slated Maps Stream Chunks to IPC

In `crates/slated/src/agent/config.rs`, the `run_stream<M>()` function maps aisdk chunks to
`StreamChunk` variants (defined in `crates/slate-common/src/messages.rs`):

| aisdk `LanguageModelStreamChunkType` | slated `StreamChunk` |
|--------------------------------------|----------------------|
...
over the Unix socket as framed bincode to the slate client.
...
Slated adds its own retry layer in `config.rs` (3 retries with `classify_error()`).

Stream errors surface as `LanguageModelStreamChunkType::Failed(String)` — slated's
...
4. **Agent ownership in slated:** The `Agent` is moved out of `ClientSession` during
   streaming and returned via a oneshot channel. Don't try to access it concurrently.

5. **Fork before tokio:** slated forks for daemonization BEFORE creating the tokio runtime.
...
7. **Tool result truncation:** slated truncates tool results to 256KB before sending back
