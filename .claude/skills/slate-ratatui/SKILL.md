...
name: slate-ratatui
description: >
  Activate this skill whenever someone asks about the **slate** Rust TUI client and their
  question involves the terminal interface. "slate" (not "slated", not "streamdown-rs") is a
  terminal app built with ratatui, crossterm, tui-term, and vt100. Any question about slate
  ...
  will rarely say "ratatui" — they'll say "slate" plus describe a visual, interactive, or
  terminal problem. That combination is your trigger. Do NOT use for: slated daemon internals,
...
# Slate TUI — ratatui Guide

This skill covers ratatui usage exclusively in the **slate** crate (the TUI client).
The slated daemon has no ratatui code — see the `slate-aisdk` skill for daemon work.

## Slate Source Layout

All UI source lives under `crates/slate/src/`:
...
`DaemonMessage` variants (from slate-common) and their render effects:
